...
def test_deposit_property_when_valid():
    # GIVEN
    game = GameFactory()
    ...
    # WHEN
    deposit_property, status = DepositService().deposit_property(user_id=player.user.id, game_id=game.id, position=card.position)
    # THEN
...
@pytest.mark.django_db(transaction=True)
def test_deposit_property_when_not_owner():
    # GIVEN
    game = GameFactory()
    card = CardFactory(position=1)
    player = PlayerFactory(game_id=game.id)
...
@pytest.mark.django_db(transaction=True)
def test_deposit_property_when_already_deposited():
    # GIVEN
    game = GameFactory()
    card = CardFactory(position=1)
    player = PlayerFactory(game_id=game.id)
